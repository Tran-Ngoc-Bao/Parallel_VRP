...
printf "run,final_global_best_cost,total_wall_time_sec\n" > "${CSV_FILE}"

for ((i=1; i<=RUNS; i++)); do
    TMP_LOG="$(mktemp)"
    bash "${RUN_SCRIPT}" > "${TMP_LOG}" 2>&1

    RESULT="$(sed -n 's/^\[Master\] Final global_best_cost: \([0-9.][0-9.]*\) (received from worker .*$/\1/p' "${TMP_LOG}" | tail -n 1)"
    TOTAL_TIME="$(sed -n 's/^Total wall time = \([0-9.][0-9.]*\) s$/\1/p' "${TMP_LOG}" | tail -n 1)"
...

{
    ...
    echo "average_final_global_best_cost=${AVG_RESULT}"
    echo "average_total_wall_time_sec=${AVG_TIME}"
    echo "best_final_global_best_cost=${BEST_RESULT}"
    echo "best_final_global_best_cost_run=${BEST_RUN}"
    echo "best_total_wall_time_sec=${BEST_TIME}"
    echo "csv_file=${CSV_FILE}"
} > "${SUMMARY_FILE}"

echo "Average total wall time (s): ${AVG_TIME}"
echo "Average final global best cost: ${AVG_RESULT}"
echo "Best final global best cost: ${BEST_RESULT}"
echo "Saved: ${CSV_FILE}"
